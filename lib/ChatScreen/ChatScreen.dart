...
class _ChatScreenState extends State<ChatScreen> {
                  ...
                  builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (index < snapshot.data!.docs.length - 1) {//It checks if the current message is not the last one in the list to avoid an index out of bounds error.
                        ...
                        if (index == snapshot.data!.docs.length - 1 ||
                            currentMessageDateTime.year != nextDateTime?.year || currentMessageDateTime.month != nextDateTime?.month || currentMessageDateTime.day != nextDateTime?.day) {
                          // Messages are from a different day
                          if (currentMessageDateTime.year == DateTime.now().year && currentMessageDateTime.month == DateTime.now().month && currentMessageDateTime.day == DateTime.now().day)
                          {
                            messageDate = 'Today';
                          } else if (currentMessageDateTime.year == DateTime.now().year &&
                              currentMessageDateTime.month == DateTime.now().month &&
                              currentMessageDateTime.day == DateTime.now().day - 1) {
                            messageDate = 'Yesterday';
                          } else {
                            messageDate = DateFormat('dd-MM-yyyy').format(currentMessageDate.toDate());
                          }
                        }
                        return Column(
                          children: [
                            ...
                            Container(
                              // Container for particular message either text message or Image message
                                alignment: (snapshot.data!.docs[index]['senderId'] == _currentUserId)
                                    ? Alignment.centerRight
                                    : Alignment.centerLeft,
                                margin: EdgeInsets.fromLTRB(0, 0, 0, 0),
                                child: (snapshot.data!.docs[index]['senderId'] == _currentUserId)
                      ...
                      child: GestureDetector(
                          onTap:(){
                              Navigator.push(context, MaterialPageRoute(builder: (_) {
                                return FullImageWidget(imageUrl: snapshot['photoUrl'],);
                              }));
                          } ,
                          child: Hero(tag: "imageHero",child: CachedChatImage(imageUrl: snapshot['photoUrl'],height: 250,width: 250,radius: 10, isRound: false,fit: BoxFit.cover,)))),
            ],
          ),
          Row(
            mainAxisSize: MainAxisSize.min,
          ...
          Row(
            ...
            children: [
              (snapshot['type'] == 'text')
                  ...
                  : CachedChatImage(imageUrl: snapshot['photoUrl'],height: 250,width: 250,radius: 10, isRound: false,fit: BoxFit.cover,),
              SizedBox(
                width: 20,
              )
            ],
  ...
  Widget chatControls(ImageUploadProvider imageUploadProvider) {
    void pickImageMethod({required ImageSource source}) async {
      ...
      firebaseStorageMethod.uploadImage(selectedImage, widget.receiver.uid, _currentUserId, imageUploadProvider);
    }
